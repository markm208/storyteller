...
var autoGeneratedDeveloperId = 1;

//an object with all of the developers keyed by their id
var allDevelopers = {};

//add the anonymous developer
allDevelopers[anonDeveloper.id] = anonDeveloper;

//create an anonymous developer group with the anonymous dev in it
var anonDevgroup = {
	id: "devGroupId-0", 
	memberIds: [anonDeveloper.id]
}
//used to assign values to developer groups
var autoGeneratedDeveloperGroupId = 1;
 ...
 */
function createFile(newFilePath, newFileName, newFileParentPath, timestamp = new Date().getTime()) {

    //we ignore changes to hidden file/dirs 
    if(!isHiddenFileOrDirInPath(newFilePath)) {
        
        //create an id for this file
        var newFileId = "fileId-" + autoGeneratedFileId;
 ...
 */
function createDirectory(newDirPath, newDirName, newDirParentPath, timestamp = new Date().getTime()) {

    //we ignore changes to hidden file/dirs 
    if(!isHiddenFileOrDirInPath(newDirPath)) {

        //create an id for this dir
        var newDirId = "dirId-" + autoGeneratedDirId;
 ...
 */
function renameFile(newFilePath, newFileName, oldFilePath, timestamp = new Date().getTime()) {
 ...
 */
function renameDir(newDirPath, newDirName, oldDirPath, timestamp = new Date().getTime()) {
 ...
 */
function moveFile(newFilePath, newFileParentPath, oldFilePath, oldFileParentPath, timestamp = new Date().getTime()) {
 ...
 */
function moveDir(newDirPath, newDirParentPath, oldDirPath, oldDirParentPath, timestamp = new Date().getTime()) {
 ...
 */
function deleteFileOrDirectory(deletedPath, timestamp = new Date().getTime()) {
	
    //we ignore changes to hidden file/dirs 
    if(!isHiddenFileOrDirInPath(deletedPath)) {
 ...
 */
function deleteFile(deletedFilePath, timestamp = new Date().getTime()) {

    //we ignore changes to hidden file/dirs 
    if(!isHiddenFileOrDirInPath(deletedFilePath)) {
 ...
 */
function deleteDir(deletedDirPath, timestamp = new Date().getTime()) {

    //we ignore changes to hidden file/dirs 
    if(!isHiddenFileOrDirInPath(deletedDirPath)) {
 ...
 */
function insertText(filePath, insertedText, startRow, startCol, isPaste, pastedInsertEventIds, timestamp = new Date().getTime()) {
 ...
 * The shadow data structure that mimicks the state of the editor is also updated. 
 */
function deleteText(filePath, startRow, startCol, numElementsToDelete, timestamp = new Date().getTime()) {
...
function createDeveloper(firstName, lastName, email) {
    ...
    if(getDeveloperByEmail(email) === null) {
        ...
        var newDeveloper = {
            id: "devId-" + autoGeneratedDeveloperId,
            firstName: firstName,
            lastName: lastName,
            email: email
        };		
...
function createDeveloperGroup(developerIds) {
	...
	var newDevGroup = {
		id: "devGroupId-" + autoGeneratedDeveloperGroupId,
